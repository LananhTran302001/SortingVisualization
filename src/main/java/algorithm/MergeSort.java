...
public class MergeSort {
    ...
    public static void startMergeSort(RectNodeArray rectArr, boolean byAscendOrder, HBox rectLine) {
        ...
        RectNodeArray rightArr = new RectNodeArray(rectArr, mid + 1, right_end - 1);

        SequentialTransition mergeSort = MergeAnimation.getMerge(left_start, leftArr, rightArr, rectArr, byAscendOrder, rectLine);
    ...
    private static void setNextMerge(RectNodeArray rectArr, boolean byAscendOrder, HBox rectLine) {
        if (current_size < rectArr.size() || left_start < rectArr.size() - 1) {
            ...
            SequentialTransition nextMerge = MergeAnimation.getMerge(left_start, leftArr, rightArr, rectArr, byAscendOrder, rectLine);
